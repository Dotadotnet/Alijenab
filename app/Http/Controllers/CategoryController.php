...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (count(Category::where(['nameEn' => $nameEn])->get())) {
            return redirect()->back()->with('error',  "شناسه انگلیسی $nameEn تکراری است");
        }

        $path = $request->file('image')->storeAs(
            'img/categories',
            $nameEn . '.' . $request->file('image')->extension()
        );
        $url_image = $path;
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($data_update['nameEn']) {
            $nameEn = $data_update['nameEn'];
        } else {
            $nameEn =  GoogleTranslate::trans($data_update['name'], 'en', 'fa');
        }
        ...
        if ($request->hasFile('image')) {
            Storage::delete($this_category->img);
            $path = $request->file('image')->storeAs(
                'img/categories',
                $nameEn . '.' . $request->file('image')->extension()
            );
        ...
        Category::where('id', $this_category->id)->update($data_update);
        session()->flash('status', 'successful');
        return redirect()->back();
    }
